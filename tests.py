...
class BdTunnelerTests( unittest.TestCase ):
  ...
  def test_login_DEV(self):
    '''
    Tests access & storage of authentication cookie info.
    Note: There are two differences between production and development:
          1) The bd-api-url, & 2) the login 'command' parameter value.
          The login function examines the url & uses the appropriate parameter value.
    '''
    settings = {
      ...
      u'UNIVERSITY_CODE': module_settings.LEGIT_UNIVERSITY_CODE }
    bd = BD_Tunneler( settings )
    assert bd.BD_API_URL == module_settings.BD_API_URL_DEV, bd.BD_API_URL
    assert bd.UNIVERSITY_CODE == module_settings.LEGIT_UNIVERSITY_CODE, bd.UNIVERSITY_CODE
    bd.login()
    assert bd.login_url == module_settings.TEST_BD_URL_AUTH_FULL_DEV, bd.login_url
    assert len(bd.login_response) > 100, bd.login_response  # html, which we don't care about
    assert type(bd.cookies_recent) == dict, type(bd.cookies_recent)
    assert bd.cookies_recent.keys() == [u'JSESSIONID'], bd.cookies_recent.keys()
  ...
  def test_initiateIsbnSearch_DEV(self):
    bd = BD_Tunneler()
    bd.BD_API_URL = module_settings.BD_API_URL_DEV
    bd.PATRON_BARCODE = module_settings.LEGIT_PATRON_BARCODE
    bd.UNIVERSITY_CODE = module_settings.LEGIT_UNIVERSITY_CODE
    bd.login()
    assert bd.cookies_recent.keys() == [u'JSESSIONID'], bd.cookies_recent.keys()
  ...
  def test_monitorIsbnSearch_DEV(self):
    settings = {
      u'BD_API_URL': module_settings.BD_API_URL_DEV,
      u'PATRON_BARCODE': module_settings.LEGIT_PATRON_BARCODE,
      u'UNIVERSITY_CODE': module_settings.LEGIT_UNIVERSITY_CODE,
  ...
  def test_checkRecords_DEV(self):
    import json
    settings = {
      u'BD_API_URL': module_settings.BD_API_URL_DEV,
      u'PATRON_BARCODE': module_settings.LEGIT_PATRON_BARCODE,
  ...
  def test_RequestInitiate_DEV(self):
    '''
    As of 2012-06 will _not_ actually submit a request
    '''
    import json
    settings = {
      u'BD_API_URL': module_settings.BD_API_URL_DEV,
